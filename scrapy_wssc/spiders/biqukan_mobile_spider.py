...
class biqukan_mobile_spider(scrapy.Spider):
    ...
    def __init__(self, bid=None):
        ...
        self.start_urls = ['http://m.biqukan.com/sort/3_1/'] #都是小说  3
    ...
    def parse(self, response):
        ...
        for li in book_list:
            ...
            yield scrapy.Request(url=u'http://m.biqukan.com'+book_href,callback=self.get_book_info, meta={"cateId":3,"book_href":book_href})
    ...
    def get_book_content(self,response):
        ...
        bookSubContentItem = BookSubContentItem();
        ...
        if re.match(pattern1, book_content_href, flags=0):
            ...
            bookSubContentItem['id'] = pattern2.search(book_content_href).group(3)
            bookSubContentItem['title'] = soup.select('div[class="header"] span[class="title"]')[0].string.split(u'_')[0].strip()
            bookSubContentItem['bookId'] = response.meta['bookId']
            bookSubContentItem['content'] = soup.find('div', id="chaptercontent").get_text()
            bookSubContentItem['linkUrl'] = response.request.url
            bookSubContentItem['createDate'] = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            yield bookSubContentItem
